...
SMS_HOOK_SECRET = os.environ.get("SMS_HOOK_SECRET")  # naya: SMS webhook ko lock karne ke liye

# BOT_TOKEN aur SMS_HOOK_SECRET dono zaroori hain, warna server error dekar ruk jayega.
if not BOT_TOKEN or not SMS_HOOK_SECRET:
    print("❌ ERROR: BOT_TOKEN ya SMS_HOOK_SECRET gayab hai! Render ke Environment Variables me set karo.")
    print("   SMS_HOOK_SECRET ek random lamba string ho (jaise: openssl rand -hex 20)")
    sys.exit(1)

BOT_TOKEN = BOT_TOKEN.strip()
SMS_HOOK_SECRET = SMS_HOOK_SECRET.strip()
...
# Order kitni der tak valid rahega (seconds). Invoice text isi se match hona chahiye.
QR_EXPIRY_SECONDS = int(os.environ.get("QR_EXPIRY_SECONDS", "600"))  # 10 minute
